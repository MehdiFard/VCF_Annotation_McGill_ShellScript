...
# cut -d"," -f1 $INPUT | sed '1d' | ts $VCF_DIR | tr -d ' ' > $DIR/vcfnames.list # moreutils pkg needed for this line (sudo apt install moreutils).
...
PicardCommandLine MergeVcfs I=$DIR/vcfnames.list O=$DIR/merged.vcf.gz 2> $LOG/merge.log  #/dev/null # Merging VCF files using Picard
...
vcftools --gzvcf $DIR/merged.vcf.gz --missing-indv --out $DIR/miss_data_per_inds 2> $LOG/miss_per_inds.log  #/dev/null # Missingness on a per-individual basis.
awk '$5 > $Missing_per_Inds {print $1}' $DIR/miss_data_per_inds.imiss | sed '1d' > $DIR/high_miss_inds # Making a list of Inds with missing genetypes rate higher than defined thrshold.
vcftools --gzvcf $DIR/merged.vcf.gz --remove $DIR/high_miss_inds --max-missing $Missing_per_Mrks --maf $MAF --hwe $HWE --recode --recode-INFO-all --out $DIR/merged_qcok.vcf --stdout | bgzip -c > $DIR/merged_qcok.vcf.gz 2> $LOG/qc.log # QC
